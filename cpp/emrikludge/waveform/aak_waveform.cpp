...
// Peters-Mathews Coefficients for Keplerian harmonic n
// Frequency = n * MeanMotion
PMCoeffs compute_pm_coeffs_exact(int n, double e) {
    if (n == 0) return {0.0, 0.0, 2.0}; // DC term (Memory)

    // Bessel argument for m=2 mode is dominated by 2*e
    // See Barack & Cutler 2004 / Chua 2017
    double x = 2.0 * e; 
    ...
    PMCoeffs res;
    // Exact formulas (algebraically simplified to remove n in denominator)
...
generate_aak_waveform_cpp(
    ...
    for (size_t i = 0; i < N; ++i) {
        ...
        // Keplerian Mean Motion (for amplitude scaling)
        ...
        double ap = 1.0 + cosi*cosi;
        double ac = 2.0 * cosi;
        double sp = 1.0 - cosi*cosi;

        // Peters-Mathews Summation
        // Sum over Keplerian harmonic n
        // Map to AAK frequencies:
        // n corresponds to the sideband k = n - 2
        // Phase = 2*Phi_phi + (n-2)*Phi_r
        
        // Range: n=1 to 50 (PM spectrum decays for large n)
        for (int n = 1; n <= 40; ++n) {
            PMCoeffs pm = compute_pm_coeffs_exact(n, e_val);
            
            // Optimization: skip small terms
            if (std::abs(pm.a) < 1e-9 && std::abs(pm.b) < 1e-9) continue;

            // AAK Phase Mapping (The "Augmented" Step)
            // Fundamental Frequency: 2*Omega_phi
            // Sideband: (n-2)*Omega_r
            double k_sideband = (double)n - 2.0;
            
            double phase = 2.0 * Phi_phi[i] + k_sideband * Phi_r[i];
            double wave_phase = phase - 2.0 * viewing_phi;
            
            double c2 = cos(wave_phase);
            double s2 = sin(wave_phase);
            
            // Polarization tensors (Source frame)
            // h+
            ...
            // Note: We ignore the m=0 memory term (pm.c) here as it is DC-like 
            // and not relevant for LISA detection band in this context.
        }
    }
